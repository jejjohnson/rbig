"""AnnealedRBIG: Modern RBIG implementation using ECDF-based marginal Gaussianization.

This module contains the primary RBIG implementation from the snippets, using an
ECDF-based approach for marginal Gaussianization.  The old RBIG class (histogram
+ interp1d approach) is retained in ``rbig._src.base`` and is used only for
salvaged features such as full Jacobian computation.
"""
...
# ---------------------------------------------------------------------------
# Marginal Gaussianization helpers (ECDF-based, modern snippets approach)
# ---------------------------------------------------------------------------
...
def fit_marginal_params(X, extension=10, n_quantiles=1000):
    ...
    Returns a params dict that is compatible with ``compute_jacobian`` (stores
    both array form for Jacobian access and callable form for fast transforms).
    ...
    # ECDF on sorted data
    sorted_x = np.sort(X)
    ecdf_y = np.arange(1, n + 1) / n  # values in (1/n, 1]

    # Extend ECDF with 0 at lb and 1 at ub
    x_ext = np.concatenate([[lb], sorted_x, [ub]])
    y_ext = np.concatenate([[0.0], ecdf_y, [1.0]])

    # Interpolate onto a fixed support
    ...
    # Empirical PDF = gradient of CDF
    pdf_vals = np.gradient(cdf_vals, support)
    pdf_vals = np.maximum(pdf_vals, 1e-15)

    # Extend PDF with zeros at boundaries (jacobian compat)
...
# ---------------------------------------------------------------------------
# AnnealedRBIG — the primary RBIG class (from the snippets)
# ---------------------------------------------------------------------------


class AnnealedRBIG(BaseEstimator, TransformerMixin):
    """Rotation-Based Iterative Gaussianization (RBIG) with ECDF marginals.

    This is the primary RBIG implementation.  It uses an empirical CDF (ECDF)
    for marginal Gaussianization at each layer, which is more robust than the
    histogram-based approach in the legacy :class:`rbig._src.base.RBIG`.
    ...
    def fit(self, X):
        ...
        for layer in range(self.n_layers):
            ...
            # --- Marginal Gaussianization (ECDF-based) ---
            layer_params = []
    ...
    def score_samples(self, X):
        ...
        for layer_params, rotation in zip(self.gauss_params, self.rotation_matrix):
            for idim, params in enumerate(layer_params):
                ...
                # log|dz/dx| = log φ(z) − log p̂(x)
                log_det_jac += log_pz - log_px
                X[:, idim] = z

            X = np.dot(X, rotation)

        # Final Gaussian log-prob
